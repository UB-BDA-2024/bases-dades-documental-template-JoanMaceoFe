...
def create_sensor(db: Session, mongodb_client: MongoDBClient, sensor: schemas.SensorCreate) -> models.Sensor:
    ...
    mongodb_document = {"name": sensor.name, "latitude": sensor.latitude, "longitude": sensor.longitude,
...
def get_data(redis: Session, sensor_id: int) -> schemas.Sensor:
    ...
    json = {"id": id_sensor, "name": name, "battery_level": battery_level, "last_seen": last_seen}
    ...
    if temperature is not None:
        json["temperature"] = float(temperature)
    if velocity is not None:
        json["velocity"] = float(velocity)
    return json 
